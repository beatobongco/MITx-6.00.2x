...
def r_squared(y, estimated):
    ...
    """
    # TODO
    squared_errors = sum([(yi - ei)**2 for yi, ei in zip(y, estimated)])
    mean = np.mean(y)
    variabilities = sum([(yi - mean)**2 for yi in y]) # variability in dependent variable
    return  1 - (squared_errors / variabilities)
...
def evaluate_models_on_training(x, y, models):
    ...
    Returns:
        None
    """
    best_fit = None # (r_squared, y_values, degree)
    for model in models:
        estimated_y = np.polyval(model, x)
        r_sq = r_squared(y, estimated_y)
        try:
            if r_sq < best_fit[0]:
                best_fit = (r_sq, estimated_y, len(model) - 1)
        except TypeError:
            best_fit = (r_sq, estimated_y, len(model) - 1)

    print(best_fit)
    plt.title('Degree: ' + str(best_fit[2]) + ' R^2: ' + str(best_fit[0]))
    plt.xlabel('years')
    plt.ylabel('temprature')
    plt.plot(x, y, marker='o', color='b')
    plt.plot(x, best_fit[1], color='r')
    plt.show()
